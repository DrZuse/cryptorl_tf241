...
class Recorder(Process):
    ...
    def timer_worker(self,
        ...
        if coinbaseClient.book.done_warming_up & \
            ...
            #LOGGER.info(f'coinbaseClient.book: \n {coinbaseClient.book}')
            ...
            #LOGGER.info(f'coinbaseClient.book.render_book(): \n {coinbaseClient.book.render_book()}')
            self._render_book_dict[dt.now()] = coinbaseClient.book.render_book()
            

            #LOGGER.info(f'render_lob_feature_names: {render_lob_feature_names()}')
            LOGGER.info(f'len(self._render_book_dict): {len(self._render_book_dict)}')
            if len(self._render_book_dict) == 100:
                render_book_dict_copy = copy.deepcopy(self._render_book_dict)
                self.observation_queue.put(render_book_dict_copy)
                self._render_book_dict.clear()
...
def main(observation_queue):
    LOGGER.info(f'Starting recorder with basket = {BASKET}')
    for coinbase, bitfinex in BASKET:
        Recorder((coinbase, bitfinex)).start()
        LOGGER.info(f'Process started up for {coinbase} and {bitfinex}')
        #LOGGER.info('sleep 9 seconds # why???')
        #time.sleep(9)
...
if __name__ == "__main__":
    ...
    main(observation_queue)

    agent = Agent(
        training=False,
        window_size=100,
        max_position=5,
        fitting_file='BTC-USD_2021-01-22.csv.xz',
        testing_file=observation_queue,
        symbol='BTC-USD',
    ...
    '''
    while True:
        while observation_queue.empty():
            print('empty')
            time.sleep(5)
        print(f'observation_queue.get(): \n{observation_queue.get()}')
    '''