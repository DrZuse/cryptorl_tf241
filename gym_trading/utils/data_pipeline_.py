...
from configurations import DATA_PATH, EMA_ALPHA, INCLUDE_ORDERFLOW, LOGGER, MAX_BOOK_ROWS, TIMEZONE
...
class DataPipeline(object):
    ...
    @staticmethod
    def import_observation_queue(testing_file: dict) -> pd.DataFrame:
        _feature_names_list = OrderBook.render_lob_feature_names(INCLUDE_ORDERFLOW)
        LOGGER.info('wait observation_queue to get()')
        data = pd.DataFrame.from_dict(data=testing_file.get(), orient='index', columns=_feature_names_list)
        data.index.name = 'system_time'
        return data
    ...
    def load_environment_data(self, fitting_file: str, testing_file: str,
        ...
        """
        LOGGER.info('load_environment_data here')
        ...
        # Fit the scaler 
        ## my: scaler fitting only on fit data -> do we need this for test ???
        ...
        data = self.import_observation_queue(testing_file=testing_file)
        LOGGER.info(f'import_observation_queue shape: {data.shape}')
