...
class BaseEnvironment(Env, ABC):
    ...
    def __init__(self,
        ...
        assert reward_type in VALID_REWARD_TYPES, \
            f'Error: {reward_type} is not a valid reward type. Value must be in:\n{VALID_REWARD_TYPES}'
        ...
        for window in INDICATOR_WINDOW:
            self.tns.add((f'tns_{window}', TnS(window=window, alpha=self.ema_alpha)))
            self.rsi.add((f'rsi_{window}', RSI(window=window, alpha=self.ema_alpha)))
        '''
        ...
        self._normalized_data = np.array([])
        self._midpoint_prices = np.array([])
        self._best_bids = np.array([])
        self._best_asks = np.array([])
        ...
        self._render.reset_render_data(y_vec=y_vec)
        '''
        # rendering class
        self._render = TradingGraph(sym=self.symbol)


    def data_for_simulations(self):
        # get historical data for simulations
        self.data_pipeline = DataPipeline(alpha=self.ema_alpha)
        ...
        self._midpoint_prices, self._raw_data, self._normalized_data = \
            self.data_pipeline.load_environment_data(
                fitting_file=self.fitting_file,
                testing_file=self.testing_file,
                include_imbalances=True,
                as_pandas=True,
            )

        LOGGER.info(f'self._midpoint_prices.shape:{self._midpoint_prices.shape}')
        LOGGER.info(f'self._midpoint_prices:\n{self._midpoint_prices}')

        LOGGER.info(f'self._raw_data.shape:{self._raw_data.shape}')
        LOGGER.info(f'self._raw_data:\n{self._raw_data}')

        LOGGER.info(f'self._normalized_data.shape:{self._normalized_data.shape}')
        LOGGER.info(f'self._normalized_data:\n{self._normalized_data}')
    ...
    def step(self, action: int = 0) -> (np.ndarray, np.ndarray, bool, dict):
        ...
        LOGGER.info('step now')
        LOGGER.info(f'action: {action}')
        self.data_for_simulations()

        for current_step in range(self.action_repeats):
            LOGGER.info(f'current_step: {current_step}')
            LOGGER.info(f'local_step_number: {self.local_step_number}')
            self.local_step_number = current_step
            ...
            LOGGER.info(f'step_action: {step_action}')
            ...
            _my_foo = (self.broker.realized_pnl * 100) / self.max_position
        ...
        LOGGER.info(f'self.max_steps: {self.max_steps}')
        ...
        if self.local_step_number > self.max_steps:
            ...
            # store for visualization after the episode
            '''
            _my_foo2 = (self.broker.realized_pnl * 100) / self.max_position
            self.viz.add(self.midpoint,  # arguments map to the column names in _init_
                         had_long_positions,
                         had_short_positions,
                         self.broker.net_inventory_count,
        ...
        LOGGER.info(f'self.observation.shape: {self.observation.shape}')
    ...
    def reset(self) -> np.ndarray:
        ...
        for step in range(self.action_repeats):
            LOGGER.info(f'local_step_number: {self.local_step_number}')
            #LOGGER.info(f'step: {step} in range 0 - {self.window_size + INDICATOR_WINDOW_MAX + 1}')
            LOGGER.info(f'step: {step} in range 0 - {self.action_repeats}')
            self.midpoint = self._midpoint_prices[self.local_step_number]
    ...
    def _create_indicator_features(self) -> np.ndarray:
        ...
        """
        _my_foo3 = (*self.tns.get_value(), *self.rsi.get_value())
        return np.array(_my_foo3, dtype=np.float32).reshape(1, -1)
    ...
    def _get_step_observation(self, step_action: int = 0) -> np.ndarray:
        ...
        #LOGGER.info(f'step_action_features: {step_action_features}')
        #LOGGER.info(f'self.step_reward: {self.step_reward}')
        observation = np.concatenate((step_environment_observation,
                                      ...
                                      self.step_reward),
                                      axis=None)
        #LOGGER.info(f'step_environment_observation.shape: {step_environment_observation.shape}')
        #LOGGER.info(f'step_indicator_features.shape: {step_indicator_features.shape}')
        #LOGGER.info(f'step_position_features.shape: {step_position_features.shape}')
        #LOGGER.info(f'step_action_features.shape: {step_action_features.shape}')
        #LOGGER.info(f'self.step_reward: {self.step_reward}')
