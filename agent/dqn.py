...
from my_rl.agents.dqn import DQNAgent
from my_rl.memory import SequentialMemory
from my_rl.callbacks import FileLogger, ModelIntervalCheckpoint
...
class Agent(object):
    ...
    def _create_cnn_model(self) -> Sequential:
        ...
        :return: keras model
        """
        #features_shape = (self.memory_frame_stack, *self.env.observation_space.shape) # (1, 100, 121)
        features_shape = (*self.env.observation_space.shape, self.memory_frame_stack) # (100, 121, 1)
        #LOGGER.info(f'self.memory_frame_stack: {self.memory_frame_stack}')
        #LOGGER.info(f'self.env.observation_space.shape: {self.env.observation_space.shape}')
        #LOGGER.info(f'features_shape: {features_shape}')

        model = Sequential()
        conv = Conv2D
        model.add(conv(input_shape=features_shape, # CPU & GPU - channels_first | CPU - channels_last
                       filters=5, kernel_size=[10, 1], padding='same', activation='relu',
                       strides=[5, 1], data_format='channels_last')) # NHWC tensor format
        model.add(conv(filters=5, kernel_size=[5, 1], padding='same', activation='relu',
                       strides=[2, 1], data_format='channels_last')) # NHWC tensor format
    ...
    def start(self) -> None:
        ...
        if self.train:
            ...
            checkpoint_weights_filename = os.path.join(self.cwd,
                                                       'dqn_weights',
                                                       step_chkpt)
            LOGGER.info(f"checkpoint_weights_filename: {checkpoint_weights_filename}")
